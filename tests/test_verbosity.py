...
def test_verbose_network():
    ...
    INPUT_DEMAND = os.path.join(EXAMPLES_DIR, "demand", "sample")
...
def test_minimal_network():
    """
    Ensure that a network with only the required fields will run.
    """
    EXAMPLES_DIR = os.path.join(os.getcwd(), "fasttrips", "Examples")

    INPUT_NETWORK = os.path.join(EXAMPLES_DIR, "networks", 'minimal')
    INPUT_DEMAND = os.path.join(EXAMPLES_DIR, "demand", "sample")
    OUTPUT_DIR = os.path.join(EXAMPLES_DIR, "output")
